        ...
        if(val){
            $.getScript(window["PATHS_PASSPORT"] + "/?ct=ajax&ac=ajax_get_mobile&account="+encodeURIComponent(val)+"&js_return=" + key, function(){
                ...
                if(res && res.state){
                    ...
                    msg = '<i class="ico-mobile"></i>您可以用手机号' + res.message + '登录';

                }
                else{
                    //Show no mobile
                    msg = '<i class="ico-mobile"></i>你的帐号还没有绑定手机号，<a href="' + window["PATHS_PASSPORT"] + '/?ct=mobile&ac=bind">立即绑定</a>';
                }
                var par = input.parent();
                var hint = par.find(".popup-hint");
                if(!hint.length){
...
H.Introduce = (function(){
    ...
    return {
        Play: function(){
            ...
            html.push('<a href="javascript:;" class="close">关闭</a><div class="player-wrap">');
            if($.browser.msie){
