...
define('AUTO_THICKBOX_PLUS_VERSION', '1.4');
...
class auto_thickbox {
	...
	function scripts() {
		if ( $this->options['builtin_res'] == 'off' ) {
			...
			wp_register_script('thickbox', $this->plugins_url($this->thickbox_js, __FILE__), array('jquery'), AUTO_THICKBOX_PLUS_VERSION);
		}
...
SCRIPT;
	...
	function add_auto_thickbox_links($links, $file) {
		if ( $file == plugin_basename(__FILE__) ) {
			$links[] = '<a href="plugin-install.php?tab=plugin-information&plugin=auto-thickbox-plus&TB_iframe=true" class="thickbox" title="' . AUTO_THICKBOX_PLUS . '">' . $this->texts['details'] . '</a>';
			$links[] = '<a href="http://wordpress.org/tags/auto-thickbox-plus" target="_blank">' . __('Support', 'auto-thickbox') . '</a>';
			$links[] = '<a href="' . __('http://attosoft.info/blog/en/', 'auto-thickbox') . 'contact/" target="_blank">' . $this->texts['contact'] . '</a>';
