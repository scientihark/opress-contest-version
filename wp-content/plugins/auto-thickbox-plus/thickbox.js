			...
			if(imageGroup){
				...
				for (TB_Counter = 0; ((TB_Counter < TB_TempArray.length) && (TB_NextHTML === "")); TB_Counter++) {
					...
					if (!(TB_TempArray[TB_Counter].href == url)) {
						if (TB_FoundURL) {
							TB_NextCaption = tb_caption(TB_TempArray[TB_Counter]);
							TB_NextURL = TB_TempArray[TB_Counter].href;
							TB_NextHTML = "<span id='TB_next'>&nbsp;&nbsp;<a href='#'>"+thickboxL10n.next+"</a></span>";
						} else {
							TB_PrevCaption = tb_caption(TB_TempArray[TB_Counter]);
				...
				TB_FirstCaption = tb_caption(TB_TempArray[0]);
				TB_FirstURL = TB_TempArray[0].href;
				TB_LastCaption = tb_caption(TB_TempArray[TB_TempArray.length - 1]);
