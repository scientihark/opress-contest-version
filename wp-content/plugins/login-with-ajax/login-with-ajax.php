...
class LoginWithAjax {
	...
	function login(){
		...
		if( !empty($_REQUEST['log']) && !empty($_REQUEST['pwd']) && trim($_REQUEST['log']) != '' && trim($_REQUEST['pwd'] != '') ){
			...
			if ( strtolower(get_class($loginResult)) == 'wp_user' ) {
				...
				$return['result'] = true;
				$return['message'] = __(":)欢迎回来...",'login-with-ajax');
				...
				//Undefined Error
				$return['result'] = false;
				$return['error'] = __(':(喔噢，未知错误~', 'login-with-ajax');
			}
		}else{
			$return['result'] = false;
			$return['error'] = __(':(喔噢，要输入用户名和密码哦~', 'login-with-ajax');
		}
		...
		if( !empty($_REQUEST['lwa']) ) {
			...
			if ('POST' == $_SERVER['REQUEST_METHOD']) {
				...
				if ( !is_wp_error($errors) ) {
					//Success
					$return['result'] = true;
					$return['message'] = __(':)注册成功啦,快去看看邮箱吧.');
				}else{
					//Something's wrong
					$return['result'] = false;
	...
	function remember(){
		...
		if ( $result === true ) {
			...
			$return['result'] = true;
			$return['message'] = __(":)邮件已发送,快去看看邮箱吧.", 'login-with-ajax');
